...
> Creator **Chandler William Ferguson** · Org [`clarity-fintech/pyramid-oracle-grid`](https://github.com/clarity-fintech/pyramid-oracle-grid)
...
**Pyramid Oracle Grid** is written entirely in **MIS** (Moniversive Invariant Static, `.mis`) — a
...
```bash
git clone https://github.com/clarity-fintech/pyramid-oracle-grid
cd pyramid-oracle-grid
...
| `bin/misc mis/kernel/PyramidOracleGridKernel.mis --check --compact-letters` | Check a single module directly with the compiler. |

Direct compiler invocation (what the Makefile runs under the hood):

```bash
bin/misc mis/kernel/PyramidOracleGridKernel.mis --check --compact-letters
...
```
mis/kernel/PyramidOracleGridKernel.mis
mis/packs/edge_worker_stream.mis
mis/packs/helix_ingest.mis
mis/packs/hf_price_feed.mis
mis/packs/oracle_quorum.mis
mis/packs/quantum_resilient.mis
...
mis/commands/PyramidOracleGridCommandCatalog.mis
mis/commands/PyramidOracleGridCommands.mis
mis/commands/PyramidOracleGridNetworkBind.mis
...
`mis/kernel/PyramidOracleGridKernel.mis` — the kernel declares the invariants the compiler enforces on every check:
...
// Title: Pyramid Oracle Grid Kernel
// Suite: Cross-Chain Bridges & Interoperability

module PyramidOracleGridKernel {
  ...
  invariant domain_bound: domain == pyramid_oracle;
  ...
  invariant pack_quantum_resilient_flag: pack_quantum_resilient_bound == true;
  invariant pack_hf_price_feed_flag: pack_hf_price_feed_bound == true;
  invariant pack_edge_worker_stream_flag: pack_edge_worker_stream_bound == true;
  invariant pack_helix_ingest_flag: pack_helix_ingest_bound == true;
  invariant pack_oracle_quorum_flag: pack_oracle_quorum_bound == true;
```
...
// Creator: Chandler William Ferguson
// Compile: bin/misc <file>.mis --check --compact-letters
// Title: Pyramid Oracle Grid · EdgeWorkerStream
// Suite: Cross-Chain Bridges & Interoperability

module PyramidOracleGridEdgeWorkerStream {
  invariant letter_hash_bound: letter_hash_root != @0;
  invariant deep_root_moniversive: deep_root == moniversive;
  invariant settlement_chain: chain_id == 1202;
  invariant settlement_network_clrty1: settlement_network == clrty_1;
  invariant kernel_is_misc: compiler_kernel == misc;
  invariant pack_edge_worker_stream_bound: pack_bound == true;
  invariant catalog_backlink: mis_catalog_bound == true;

  invariant edge_worker_stream_041: edge_worker_stream_041_bound == true;
  invariant edge_worker_stream_042: edge_worker_stream_042_bound == true;
  invariant edge_worker_stream_043: edge_worker_stream_043_bound == true;
  invariant edge_worker_stream_044: edge_worker_stream_044_bound == true;
  invariant edge_worker_stream_045: edge_worker_stream_045_bound == true;
  invariant edge_worker_stream_046: edge_worker_stream_046_bound == true;
  invariant edge_worker_stream_047: edge_worker_stream_047_bound == true;
```
...
```
CONNECTED clrty-1/1202 pyramid-oracle-grid
RPC https://rpc.clarity-fintech.com
MODULES mis/commands/PyramidOracleGridCommands.mis
```
...
{
  "ok": true,
  "module": "PyramidOracleGridBacklinkIndex",
  "kernel": "misc",
  "invariant_count": 9,
  "outcome_count": 4,
  "typed_letters": 1211,
...
*Reproduce:* `make check` → `"ok": true`, module `PyramidOracleGridBacklinkIndex`, 9 invariants, 4 outcomes, 1211 typed letters.
...
- [clarity-fintech/pyramid-oracle-grid](https://github.com/clarity-fintech/pyramid-oracle-grid) — this repository
